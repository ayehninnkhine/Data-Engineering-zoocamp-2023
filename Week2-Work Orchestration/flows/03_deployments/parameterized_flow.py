...
def extract_from_gcs(color, year, month) -> Path:
    ...
    gcs_block = GcsBucket.load("prefect-de-gcs")
...
def write_bq(df) -> None:
    ...
    gcp_credentials_block = GcpCredentials.load("zoomcamp-de-gcp-creds")

    df.to_gbq(
        destination_table="rides_yellow.rides_yellow",
        project_id="sylvan-earth-375806",
